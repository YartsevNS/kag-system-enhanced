...
### Проблема
Embedding-модели (bge-m3, nomic-embed-text) имеют лимит токенов (~8K) и теряют точность на длинных текстах. Один embedding-вектор на 10-страничный документ даёт "размытый" результат.

### Решение (на основе анализа сообщества LangChain)
Опциональная суммаризация через LLM перед векторизацией:
1. После парсинга документ отправляется в LLM с промптом "Создай аннотацию (3-5 предложений)"
2. LLM возвращает сжатую версию (200-300 токенов)
3. Аннотация сохраняется в record.summary
4. Чанки векторизуются отдельно как обычно

### Когда использовать
- **Включать:** документы >5 страниц, поиск по общему смыслу
- **Не включать:** короткие документы, поиск по конкретным фрагментам

### Настройки
- Админ-панель -> Настройки OCR -> Суммаризация (enable_summarization)
- По умолчанию: выключено
- Сохраняется в config_store("ocr", "settings")
