...
@router.post("/add-secnews", summary="Добавить RSS SecurityNews")
async def add_secnews_sources():
    """Добавить 5 RSS-каналов SecurityNews (ИБ-новости)."""
    try:
        from src.api.services.web_monitor import web_monitor, MonitorSource, WebMonitorService
        ...
        for src in WebMonitorService.SECNEWS_SOURCES:
            if src["url"] in existing:
                continue
            source = MonitorSource(
                id=str(uuid.uuid4()),
