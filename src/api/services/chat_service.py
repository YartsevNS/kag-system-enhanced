...
# ВАЖНО про смысл числа: это допустимое ОТСТАВАНИЕ от лучшего фрагмента, а не «строгость».
# gap=0,02 — жёстко (оставляем только то, что почти не отличается от лучшего), gap=0,20 —
# мягко (оставляем всё, что отстало не больше чем на 0,20, то есть почти всю выдачу).
SCORE_GAP_MIN = 0.02          # пользовательский диапазон: 0 = выключить отсечение нельзя
SCORE_GAP_MAX = 0.12          # и «брать почти всё» (мягкий край) тоже нельзя — только админ
SCORE_GAP_BY_DOMAIN = {       # доменные дефолты: где мусор ближе к топу, отсечение жёстче
    "universal": 0.08,
    "legal": 0.05,
    "infosec": 0.03,
    "accounting": 0.03,
...
def resolve_score_gap(params: Dict[str, Any], requested: Optional[float],
    ...
    Смысл числа — допустимое отставание от лучшего фрагмента: МЕНЬШЕ = жёстче отбор.
    Значение из запроса зажато пользовательским диапазоном (0,02..0,12): выключить отсечение
    (0) или взять почти всю выдачу (мягкий край) из интерфейса нельзя. Значение из привязки —
    до 0,30, и только оно может быть 0 («выключено») — это решение администратора.
    """
    params = params or {}
