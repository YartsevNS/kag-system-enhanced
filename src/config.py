...
class Settings(BaseSettings):
    ...
    # Замер 2026-09-13 (набор из 7 вопросов, корпус 52 документа): префикс 200 символов
    # поднял покрытие (hit@3 0.857 -> 1.0, precision@10 0.63 -> 0.81) и просадил первую
    # позицию (hit@1 0.714 -> 0.571, MRR 0.821 -> 0.762): все чанки документа получают
    # ОДИН И ТОТ ЖЕ префикс, и «тема документа» начинает перевешивать сам чанк.
    # Поэтому длина и включение — настройка, а не константа: короткий префикс (title+type)
    # проверяется тем же набором, что и длинный.
    CARD_PREFIX_ENABLED: bool = True
    CARD_PREFIX_MAX_CHARS: int = 200
