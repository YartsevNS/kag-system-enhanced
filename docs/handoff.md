...
Базы для сравнения: качество ответов (LLM-судья, 20 вопросов с эталонами) **0.635**;
retrieval на 7 исторических вопросах hit@1 0.7143 / hit@3 1.0 / MRR 0.8333; покрытие
сравнительных вопросов 0.875.

1. Гипотеза ПОЛНОТЫ промпта (пользователь: «позже»): после провала кандидата «требуй конкретику»
   (0.555 против 0.635) проверить обратное — «приведи все существенные факты, номера и обозначения
   из контекста, не сокращай». Один прогон `scripts/evaluate_answers_template.py`. Механика A/B:
   промпт в БД (`function_map/chat`), меняется без пересборки; бэкап/откат —
   `reports/_scratch/prompt_ab_set.py` и `prompt_ab_restore.py` (бэкап `prompt_before.txt`).
2. Слабые ответы при факте в контексте: пентест 2-МР, ИИ на финансовом рынке, профиль защиты ПО
   (0.00) — генерация не называет факты. Дополнительные рычаги: глубина контекста top-5 вместо top-10,
   пометка лучшего фрагмента. Диагностика: `reports/_scratch/fact_rank.py` (ранг чанка с фактом).
