...
def test_db():
    ...
    # StaticPool: in-memory SQLite существует НА СОЕДИНЕНИЕ. Без него create_all
    # работает в одном соединении, а сессия роута берёт другое и не видит таблиц.
...
def client(test_db, auth_headers, monkeypatch):
    ...
    # Тесты из-за этого получали HTML вместо JSON, а проверки кодов падали.
