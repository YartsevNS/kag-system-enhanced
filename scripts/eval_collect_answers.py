...
def login() -> str:
    ...
    d = _post("/api/v1/auth/login", {"username": os.environ.get("ADMIN_USER", "admin"),
                                     "password": pw})
...
def main() -> int:
    ...
    for i, item in enumerate(items, 1):
        ...
        contexts = []
        try:
            srch = _post("/api/v1/chat/search", {"query": q, "limit": 10}, token)
        ...
        print(f"  {i}/{len(items)} {item['id']}: ответ {len(answer)} симв., "
              f"источников {len(sources)}, контекстов {len(contexts)}", flush=True)
